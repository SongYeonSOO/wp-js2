...
(function(window, undefined) {
	...
	jQuery.support = (function() {
		...
		var support, all, a, select, opt, input, fragment, eventName, isSupported, i, div = document
				.createElement("div");
	...
	jQuery.event = {
		...
		add : function(elem, types, handler, data, selector) {
			...
			// plain objects)
			elemData = elem.nodeType !== 3 && elem.nodeType !== 8
					&& jQuery._data(elem);
		...
		remove : function(elem, types, handler, selector, mappedTypes) {
			...
			var j, origCount, tmp, events, t, handleObj, special, handlers, type, namespaces, origType, elemData = jQuery
					.hasData(elem)
					&& jQuery._data(elem);
		...
		trigger : function(event, data, elem, onlyHandlers) {
			...
			var i, cur, tmp, bubbleType, ontype, handle, special, eventPath = [ elem
					|| document ], type = event.type || event, namespaces = event.namespace ? event.namespace
					.split(".")
					: [];
	...
	(function(window, undefined) {
				...
				return a.compareDocumentPosition ? -1 : 1;
					...
					: function(a, b) {
						...
						if (a === b) {
							...
							return (~b.sourceIndex || MAX_NEGATIVE)
		...
		function matcherFromGroupMatchers(elementMatchers, setMatchers) {
			...
			var matcherCachedRuns = 0, bySet = setMatchers.length > 0, byElement = elementMatchers.length > 0, superMatcher = function(
				...
				// `i` starts as a string, so matchedCount would equal "00" if
		...
		function select(selector, context, results, seed) {
			...
			if (!seed) {
				...
				if (match.length === 1) {
					...
					for (i = matchExpr["needsContext"].test(selector) ? -1
	...
	jQuery
			.extend({
				clone : function(elem, dataAndEvents, deepDataAndEvents) {
					var destElements, srcElements, node, i, clone, inPage = jQuery
				...
				buildFragment : function(elems, context, scripts, selection) {
					var contains, elem, tag, tmp, wrap, tbody, j, l = elems.length,
	...
	var curCSS, getStyles, iframe, ralpha = /alpha\([^)]*\)/i, ropacity = /opacity\s*=\s*([^)]*)/, rposition = /^(top|right|bottom|left)$/,
	...
	function ajaxConvert(s, response) {
		...
		var conv, conv2, current, tmp, converters = {}, i = 0,
